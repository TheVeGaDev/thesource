...
# Import tgbot if it exists in your config
try:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("الفحص") and event.query.user_id == zedub.uid:
            ...
            # Initialize zzd and zzt with default values
            zzd = gvarstatus("z_date") or f"{bt.year}/{bt.month}/{bt.day}"
            zzt = gvarstatus("z_time") or f"{bt.hour}:{bt.minute}:{bt.second}"
            zedda = f"{zzd}┊{zzt}"
            ...
            ALIVE_NAME = gvarstatus("ALIVE_NAME") if gvarstatus("ALIVE_NAME") else z_name
            
            # Use tepversion instead of undefined zedversion
            zdver = tepversion
...
async def help(event):
    ...
    try:
        response = await zedub.inline_query(Config.TG_BOT_USERNAME, "مساعدة")
        if response:
            await response[0].click(event.chat_id)
            await event.delete()
        else:
            await event.edit("**❌ لا توجد نتائج للاستعلام المضمن**")
